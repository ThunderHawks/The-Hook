...
/************************************************************************************************************************************************************************************************/
   ...
   PlaceModel(damesh, 4, 1, 4, 1, 1, 1, 1);
   
   //The model (hopefully)
   /*SetModel(4, 1, 4, 1, 1, 1, 1);
   safe_glEnableVertexAttribArray(h_aPosition);
   glBindBuffer(GL_ARRAY_BUFFER, damesh.PositionHandle);
   safe_glVertexAttribPointer(h_aPosition, 3, GL_FLOAT, GL_FALSE, 0, 0);

   safe_glEnableVertexAttribArray(h_aNormal);
   glBindBuffer(GL_ARRAY_BUFFER, damesh.NormalHandle);
   safe_glVertexAttribPointer(h_aNormal, 3, GL_FLOAT, GL_FALSE, 0, 0);

   /* draw!*/
  /* glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, damesh.IndexHandle);

   glDrawElements(GL_TRIANGLES, damesh.IndexBufferLength, GL_UNSIGNED_SHORT, 0);
